"""
Code to solve the problem on the circle centered in (0,0) of radius 1, for different values of l. 
...
f = open("outputs/output_euler_homo_circle_problem_l_{l}_T_{T}_dt_{exp_dt}_h_P{deg}_viariate_degPhi_{sigma}.txt".format(T=int(T),l=degPhi,deg=degV,exp_dt=int(exp_dt), sigma=sigma), "w")

#######################################
#              Exact FEM              #
#######################################

domain = mshr.Circle(df.Point(0.0,0.0), 1.)
...
for l in range(degV, degV +5):
    ...
    for i in range(0, Iter):
        ...
        if test_case == 1 : 
            ...
            while hmax > dt + df.DOLFIN_EPS : 
                ...
                mesh_macro = df.RectangleMesh(df.Point(-1.5,-1.5), df.Point(1.5,1.5), size, size)
                size+=1
        ...
        phi = df.Expression("-1. + (x[0]*x[0] + x[1]*x[1])", degree = degPhi, domain = mesh_macro)
        ...
        for ind in range(mesh_macro.num_cells()):
            ...
            if phi(v1x, v1y) <= 0.0 or df.near(phi(v1x, v1y),0.0) or phi(v2x, v2y) <=0.0 or df.near(phi(v2x, v2y),0.0) or phi(v3x, v3y)<= 0.0 or df.near(phi(v3x, v3y),0.0):
                domains[ind] = 1
        mesh = df.SubMesh(mesh_macro, domains, 1)
        print("num cell mesh:",mesh.num_cells())
        V_phi = df.FunctionSpace(mesh, "CG", degPhi)
        phi = df.Expression("-1. + (x[0]*x[0] + x[1]*x[1])", degree = degPhi, domain = mesh)
        phi = df.interpolate(phi, V_phi)
        print(
            "###########################\n"
            f"Mesh built using the following parameters : \n  "  
    ...
    if test_case == 1 : 
        plt.loglog(size_mesh_phi_fem_vec, error_H1_phi_fem_vec, label=f'H1 l={degPhi}')
        plt.loglog(size_mesh_phi_fem_vec, size_mesh_phi_fem_vec)
    elif test_case == 2 : 
        plt.loglog(size_mesh_phi_fem_vec, error_L2_phi_fem_vec, label=f'L2 l={degPhi}')
