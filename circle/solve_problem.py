"""
Code to solve the problem on the circle centered in (0,0) of radius 1. 
...
f = open("outputs/output_euler_homo_circle_problem_l_{l}_T_{T}_dt_{exp_dt}_h_P{deg}_sigma_{sigma}.txt".format(T=int(T),l=degPhi,deg=degV,exp_dt=int(exp_dt), sigma=sigma), "w")
...
for i in range(0, Iter):
    ...
    for ind in range(1, len(Time)):
        ...
        #df.solve(A == B, u_n1, bc, solver_parameters={"linear_solver": "mumps"})
        if compute_times:
            end = time.time()
